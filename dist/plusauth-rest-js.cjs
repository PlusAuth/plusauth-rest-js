...
var PlusAuthRestError = class extends Error {
	constructor(error) {
		...
		if (!(error instanceof Error)) Object.assign(this, error);
		else Object.defineProperty(this, "_raw", error);
	}
};
...
async function parseFetchResponse(response, options) {
	...
	if (options.responseType === "stream" && response.ok) return response.body?.getReader();
	if (options.responseType === "json" || contentType && contentType.indexOf("application/json") > -1) return await response.json();
...
async function fetchAsPromise(url, options) {
	...
	if (rawResponse.ok) return await parseFetchResponse(clone, options);
...
var HttpService = class {
	...
	constructor(apiURL, options = {}) {
		...
		if (options.httpClient && typeof options.httpClient !== "function") throw new Error("\"httpClient\" must be function");
		const _baseUrl = apiURL + (/\/api\/v\d(\/)?$/.test(apiURL) ? "" : `${apiURL.endsWith("/") ? "" : "/"}api/v1`) + this.constructor.prefix;
		...
		for (const method of [
			...
			let token;
			if (options && typeof options.token === "function") token = options.token.call(void 0);
			...
			if (args.length > 1) {
				if (method !== "get") fetchOptions.body = typeof args[1] === "object" ? JSON.stringify(args[1]) : args[1];
			}
			if (!!args[2] && typeof args[2] === "object") fetchOptions = deepmerge(fetchOptions, args[2]);
...
function encodedQueryString(data, appendable = true) {
	if (!data) return "";
	const ret = [];
	for (const d in data) if (data[d] != null) ret.push(`${d}=${encodeURIComponent(data[d].toString())}`);
	if (appendable) return `?${ret.join("&")}`;
	return ret.join("&");
}

//#endregion
//#region src/api/clients.ts
...
//#endregion
//#region src/api/customDomains.ts
...
//#endregion
//#region src/api/tenantAdministrators.ts
